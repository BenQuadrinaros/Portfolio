class CloudUpgrade extends Phaser.GameObjects.Sprite {
    ...
    constructor(scene, x, y, texture, list) {
        ...
        this.title = this.scene.add.text(this.x, this.y - 7 * this.height / 20, "CLOUDS", menuConfig).setOrigin(.5);
        this.title.depth = 11;
        ...
        this.scene.input.on('pointerdown', () => {
            ...
            if (!this.hidden && this.conditionsMet() && mouse.x > this.x - this.width / 2 &&
                ...
                //implement effects
                this.scene.cloudReleaseTimer *= this.effect[0];
                this.scene.cloudReleaseAmount += this.effect[1];
    ...
    displayMetrics() {
        ...
        if (this.effect[0] != 1) {
            this.reward.text += "Release Timer\n";
        }
        if (this.effect[1] > 0) {
            this.reward.text += "Release Amount\n";
        }
    }
