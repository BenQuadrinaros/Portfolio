class DropUpgrade extends Phaser.GameObjects.Sprite {
    ...
    constructor(scene, x, y, texture, list) {
        ...
        this.title = this.scene.add.text(this.x, this.y - 7 * this.height / 20, "DROPS", menuConfig).setOrigin(.5);
        this.title.depth = 11;
        ...
        this.scene.input.on('pointerdown', () => {
            ...
            if (!this.hidden && this.conditionsMet() && mouse.x > this.x - this.width / 2 &&
                ...
                if (this.scene.damMade) {
                    this.scene.maxDrops += 2 * this.effect[0];
                } else {
                    this.scene.maxDrops += this.effect[0];
                }
                this.scene.saturationChance += this.effect[1];
                this.scene.dropSaturation += this.effect[2];
    ...
    displayMetrics() {
        ...
        if (this.effect[0] > 0) {
            this.reward.text += "Maximum Drops\n";
        }
        if (this.effect[1] > 0) {
            this.reward.text += "Saturation Chance\n";
        }
        if (this.effect[2] > 0) {
            this.reward.text += "Maximum Saturation\n";
        }
    }
