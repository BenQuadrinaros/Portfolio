class LakeUpgrade extends Phaser.GameObjects.Sprite {
    ...
    constructor(scene, x, y, texture, list) {
        ...
        this.title = this.scene.add.text(this.x, this.y - 7 * this.height / 20, "LAKE", menuConfig).setOrigin(.5);
        this.title.depth = 11;
        ...
        this.scene.input.on('pointerdown', () => {
            ...
            if (!this.hidden && this.conditionsMet() && mouse.x > this.x - this.width / 2 &&
                ...
                //implement effects
                this.scene.lakeSedTake -= this.effect[0];
                this.scene.lakeConversionTimer *= this.effect[1];
                for (let i = 1; i <= this.effect[2]; ++i) {
                    this.scene.spawnCloud();
                }
    ...
    displayMetrics() {
        ...
        if (this.effect[0] > 0) {
            this.reward.text += "Sediment Cost Reduction\n";
        }
        if (this.effect[1] < 1) {
            this.reward.text += "Conversion Timer Decrease\n";
        }
        if (this.effect[2] > 0) {
            this.reward.text += "Cloud\n";
        }
    }
